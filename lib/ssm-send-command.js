            ...
            ssm.listCommandInvocations(ssmParams, function(err, data) {
                ...
                async.each(asyncParams,
                    function(asyncParam, callback) {
                      console.log(`Downloading: ${asyncParam.s3Param.Key}`);
                        s3.getObject(asyncParam.s3Param, function(err, s3Data) {

                            if (err) {
                                console.log('s3:getObject failed' + err);
                                return callback(err);
                            }

                            if (!s3Data.Body) {
                                return callback('No commnand output returned');
                            }
        ...
        var interval = setInterval(function() {
            ...
            if (!checkingStarted) {
                ...
                ssm.listCommands(ssmParams, function(err, data) {
                    ...
                    if (checks > 200 || (currentStatus == "Success")) {
...
function run_command(options, context) {
    ...
    return function(cb) {
        ...
        var params = {
            ...
            TimeoutSeconds: options.commandTimeout,
            OutputS3BucketName: options.outputS3BucketName,
            OutputS3KeyPrefix: options.outputS3KeyPrefix || "run-command"
        };
